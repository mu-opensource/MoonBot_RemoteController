...
MoonBotRemoteProtocolAnalysis::MoonBotRemoteProtocolAnalysis(MuUart::hw_port_t uart,
                                                             uint32_t address,
                                                             bool response_enable)
    : MuPtotocolAnalysis<MuUart, MuUart::hw_port_t>(uart, address),
      response_enable_(response_enable){
}
...
int MoonBotRemoteProtocolAnalysis::capture(void) {
  if (!size()) {
    ...
    err = receive();
    if (err != MU_OK) {
      // XXX currently no error is returned to slave
      EPRINTF("receive error:0x%02x\n", err);
      return err;
    }
    if (size() == 0) {
      return MU_FAIL;
...
void MoonBotRemoteProtocolAnalysis::responseError(uint8_t err) {
  if (response_enable_) {
    ...
    err_resp_ = err;
    transmit(pkg_);
#if MORPX_DEBUG_ENABLE
    EPRINTF("ackError:0x%x: \n", err);
#endif
  }
}
...
void MoonBotRemoteProtocolAnalysis::response(uint8_t cmd, uint8_t sub_cmd) {
  if (response_enable_) {
    pkg_.len = 3+parameter_offset_;
    err_resp_ = MU_SLAVE_OK;
    cmd_resp_ = cmd;
    sub_cmd_resp_ = sub_cmd;
    transmit(pkg_);
#if MORPX_DEBUG_ENABLE
    printf("\e[0;32mI:ackOK[0x%x, 0x%x] ", cmd, sub_cmd);
    for (size_t i = 0; i < pkg_.len; ++i) {
      printf("0x%x,", pkg_.buf[i]);
    }
    printf("\n\e[0m");
#endif
  }
...
void MoonBotRemoteProtocolAnalysis::cmdParameterTest(void) {
  ...
  uint8_t parameter_len = (pkg_.len>3 ? pkg_.len:3)-3;
  printf("paramter length: %hhu byte, id: 0x%02x\n",
         parameter_len, id_);
  if (parameter_len == 0) return;
  printf("paramter(uint8_t): ");

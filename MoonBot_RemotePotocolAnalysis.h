...
class MoonBotRemoteProtocolAnalysis : public MuPtotocolAnalysis<MuUart, MuUart::hw_port_t> {
 public:
  MoonBotRemoteProtocolAnalysis(MuUart::hw_port_t uart,
                                uint32_t address,
                                bool response_enable = true);
  virtual ~MoonBotRemoteProtocolAnalysis(void);
  MoonBotRemoteProtocolAnalysis& operator=(const MoonBotRemoteProtocolAnalysis &) = delete;
  ...
  int getUnuseParamterLen(void) {
    return (int)pkg_.len-3-parameter_offset_;
  }
  template<class T>
  T getParameter(void) {
    size_t len = sizeof(T);
...
//    parameter_offset_ += len;
 ...
 protected:
  pkg_t pkg_;
  uint8_t& err_resp_ = pkg_.buf[0];
  uint8_t& cmd_resp_ = pkg_.buf[1];
  uint8_t& sub_cmd_resp_ = pkg_.buf[2];
  const uint8_t& cmd_ = pkg_.buf[0];
  const uint8_t& sub_cmd_ = pkg_.buf[1];
  const uint8_t& id_ = pkg_.buf[2];
  uint8_t* const parameter_ = &pkg_.buf[3];
  int parameter_offset_ = 0;
  bool response_enable_ = true;
};
